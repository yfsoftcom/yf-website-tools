...
var ObjectConvert = function ObjectConvert(data) {
  ...
  _lodash2.default.forEach(data, function (val, key) {
    if (_lodash2.default.isString(val.result)) {
      var o = {};
      o[key] = val.result;
      list.push(o);
    } else {
...
var singlePageSpider = function singlePageSpider(item, callback) {
  new _WebXpathSelector2.default(item.url).getSelector(item.options).then(function (selector) {
    ...
    if (options.children) {
      (function () {
        var children = {};
        _lodash2.default.map(data, function (i) {
          var c = _lodash2.default.assign({}, options.children);
          c.url = c.url.replace('[?]', i[c.id || 'id']);
          c.id = i[c.id || 'id'];
          children[c.id] = c;
        });
        (0, _each2.default)(_lodash2.default.values(children), singlePageSpider, function (err) {
          _lodash2.default.map(data, function (d) {
            _lodash2.default.map(children[d.id].data, function (cd) {
              d = _lodash2.default.assign(d, cd);
            });
          });
          item.data = data;
          callback(err);
          ...
          case 0:
            ...
            _ret2 = function () {
              ...
              var items = _lodash2.default.map(pages, function (p) {
                return {
                  url: _lodash2.default.replace(url, placeholder, p),
                  xpaths: xpaths,
                  options: options
                };
              });
            ...
            return _context.abrupt('return', _ret2.v);
